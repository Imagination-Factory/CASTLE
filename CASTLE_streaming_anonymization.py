...
def best_selection(new_tuple, k, betha):
    ...
    if min_e <= tau_global:
        ...
        if len(non_ks_clusters) >= betha:
            ...
            return create_new_cluster(new_tuple, new_cluster_index = number_of_cluster_indezes)
...
def split(current_non_ks_cluster, k):
    ...
    while len(BS) >= k:
        ...
        for x in range(k-1): # iterates from 0 to k-2 meaning we get k-1 nodes into the heap
            heappush(heap, KeyDict(9999999, {'1':x})) # initialized with an infinite value
        ...
        for bucket_item in BS:
            ...
            # here we calculate distance of this tuple to the B_dash cluster
            distance_t = enlargement_calculation(new_tuple=t_picked_for_comparison, non_ks_cluster_item=t_chosen_new_cluster) 
        ...
        for i in range(k-1): # just the top k-1 nodes of the heap of interest
    ...
    print("Splitting completed. We have " + str(len(SC)) + "new clusters after the split operation.")
...
def merge_clusters(expiring_cluster, set_of_other_clusters, k):
    ...
    while calculate_size_of_cluster(merged_clusters) < k:
        ...
        print("..successfully merged cluster ID = " + str(min_enlargement_cluster.at[0, 'clusterID']) + " to the current cluster..")
...
def suppress_tuple(single_tuple):
    ...
    generalization_string = "[0-255][0-255][0-255][0-255][0-255][0-255]"
    
    output_string = output_string + generalization_string
    
    print("Output of Tuple ID =" + str(single_tuple["index"]) + ":" + str(output_string))
            ...
            if column == 'tuples_IDs':
                if single_tuple["index"] in cluster_item.at[0, column]:
                    ...
                    if len(cluster_item.at[0, column]) == 0:
                        ...
                        print("Remove cluster ID = " + str(cluster_item.at[0, 'clusterID']) + " from non-ks-cluster set.")
                        del non_ks_clusters[intermediate_index]
        intermediate_index += 1
...
def output_cluster(current_non_ks_cluster, current_size_cluster, stream_of_tuples,  k, my):
    ...
    for single_C_i in SC:
        ...
        for single_tuple_ID in single_C_i.at[0, 'tuples_IDs']:
            ...
            for i_c, column in enumerate(single_C_i):
                if column != 'tuples_IDs' and column != 'clusterID':
                    ...
                    generalization_string = generalization_string + "[" + str(min_value) + "-" + str(max_value) + "]"
            output_string = output_string + generalization_string
            print("Output of Tuple ID =" + str(current_tuple["index"]) + ":" + str(output_string))
        ...
        print("param:tau_global updated to: " + str(tau_global) + " and last generalization had info loss:" + str(info_loss_cluster(single_C_i)))
        ...
        for cluster_item in non_ks_clusters: # maybe the for loop and if are not needed if removing of cluster works
            if cluster_item.at[0, 'clusterID'] == single_C_i.at[0, 'clusterID']:
                print("We can remove the cluster ID = " + str(single_C_i.at[0, 'clusterID']) + " from the non-ks-cluster set.")
                del non_ks_clusters[intermediate_index]
            intermediate_index += 1            
